...
public class AnchorEngineConfig extends ReadyConfig {
    ...
    private transient final Map<String, Overlay> overlayMap = new ConcurrentHashMap<>();
    ...
    public void loadOverlay(){
        File folder = getFile("overlay");
        if(!folder.exists()){
            folder.mkdirs();
            getPlugin().saveResource("overlay/default.json",true);
        }
        overlayMap.clear();
        for (File file : Objects.requireNonNull(folder.listFiles((file, name) -> name.endsWith(".json")))) {
            try(FileInputStream is = new FileInputStream(file);
                ...
                json.forEach(elem -> {
                    try {
                        Overlay overlay = gson.fromJson(elem, Overlay.class);
                        if(overlay!=null) overlayMap.put(overlay.getId(),overlay);
                    } catch (JsonParseException e){
                        AnchorUtils.warn("Failed to load an overlay from "+file.getName(),e);
                    }
                });
            }catch (IOException | JsonParseException e){
                AnchorUtils.warn("Failed to load overlay from "+file.getName(),e);
            }
        }
    }
    ...
    public Optional<Overlay> getOverlay(String id){
